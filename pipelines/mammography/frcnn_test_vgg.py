...
def non_max_suppression_fast(boxes, probs, overlap_thresh=0.9, max_boxes=300):
	...
	while len(idxs) > 0:
		...
		#print("overrrrr",overlap)

		# delete all indexes from the index list that have
		idxs = np.delete(idxs, np.concatenate(([last],
			np.where(overlap > overlap_thresh)[0])))
		#print("idxxxxx",type(idx))
	...
	probs = probs[pick]
	# print("probsssssssssssss",probs.shape)
	# print("boxesssssssssssss",boxes.shape)
	# print("boxesssss",(boxes),"ppppppp",probs)
	return boxes, probs
...
def rpn_to_roi(rpn_layer, regr_layer, C, dim_ordering, use_regr=True, max_boxes=300,overlap_thresh=0.9):
	...
	# print("all_prob",all_probs.shape)
	# print("allllll",all_boxes.shape) 
...
# path='/content/drive/My Drive/Dataset/mammogram/'
# lines=[]
# f = open(path+"test.txt","r")
# for i in f:
#   x=i.replace("/content","content")
#   lines.append(x)
# f.close()
# print(lines)
...
base_path = '/content/drive/My Drive/Dataset/mammogram/'
...
test_path = base_path+'Testdir/test.txt'
...
config_output_filename = os.path.join(os.getcwd(),'pipelines/mammography/model_vgg_config.pickle')
#os.path.join(base_path,'model_vgg_config.pickle')
...
#imgs_path = []
#for i in range(30):
#	idx = np.random.randint(len(test_imgs)-2)
...
def predictt(img,img_name):
    ...
    #print("Fshapeeeeeeeeeeeeeeeeeeeeee",F[0].shape)
    #print("Y1shapeeeeeeeeee",Y1.shape)
    #print("Y2shapeeeeeeeeee",Y2.shape)

    # Get bboxes by applying NMS 
    # R.shape = (300, 4)
    R = rpn_to_roi(Y1, Y2, C, K.common.image_dim_ordering(), overlap_thresh=0.9)
    #print("Rshapeeeeeeeeeeee",R.shape)
    #for i in range(300):
      #print("AAA",R[i])
    ...
    for jk in range(R.shape[0]//C.num_rois + 1):
        ...
        if jk == R.shape[0]//C.num_rois:
            ...
            ROIs = ROIs_padded
            #print("ROISSSSSSSSSS",ROIs)
        #print("ROISSSSSSS",ROIs.shape)
        [P_cls, P_regr] = model_classifier_only.predict([F, ROIs])
        #print("PCLSSHAPE",P_cls.shape)
        #print("PREGRSHAPE",P_regr.shape)
        #for j in range(len(P_cls)):
          #print("JJJJJJJJJ",P_cls[j],"regrrrrrr",P_regr[j])
        #print("priyalllllllll",P_cls,"shapeeeeeeeeeee",P_cls.shape)#uptill here its fine
    ...
    for key in bboxes:
        ...
        #print("shpeeeeeeee",new_boxes.shape[0])
        for jk in range(new_boxes.shape[0]):
            ...
            #print("gaya3",real_x1, real_y1, real_x2, real_y2)

            cv2.rectangle(img,(real_x1, real_y1), (real_x2, real_y2), (0, 0,255),4)
...
for idx, img_data in enumerate(test_imgs):
    ...
    R = rpn_to_roi(Y1, Y2, C, K.common.image_dim_ordering(), overlap_thresh=0.6)
    ...
    print("returned t = ",t,"retuned p = ",p)
    for key in t.keys():
        if key not in T:
            T[key] = []
            P[key] = []
    ...
    all_aps = []
    for key in T.keys():
        ap = average_precision_score(T[key], P[key])#average precision score
        print('{} AP: {}'.format(key, ap))
        all_aps.append(ap)
    print('mAP = {}'.format(np.mean(np.array(all_aps))))#average of benin and maligant average precision values.
...
#print()
print('mean average precision:', np.mean(np.array(mAPs)))#average of (average of benin and maligant average precision values.)
